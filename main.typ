...
为了求解上述微分方程组，我们可以采用数值方法，如 Runge-Kutta 方法。给定初始条件（$S_0 = 999,I_0=1,R_0=0,D_0=0$）和参数（$beta=0.3,gamma=0.1,mu=0.01$），利用python语言，我们可以用数值方法计算出各个群体数量随时间变化的曲线如 @fig1 所示。

#figure( image("./figures/Figure_1.png",width: auto), caption: [传染病模型], )<fig1>

=== 分析和解释结果

从 @fig1 中可以看出，整个疫情的传播过程大体可以分为三个阶段：

1. *初期阶段*：易感者数量急剧增加，但感染者数量较少，随着时间的推移，感染者数量逐渐增加，康复者数量逐渐减少，最终康复者数量和死亡者数量达到平稳状态。
